...
from model.model_5_2_x2_add import Net_rgb_text, Net_rgb_image, net_total
...
parser.add_argument("--earlyLrStep", type=int, default=200, help="training batch size")
...
def train(training_data_loader, optimizer, model, criterion, criterion_zero, epoch, tb_writer):
    ...
    for iteration, batch in tqdm(enumerate(training_data_loader, 1), desc='train dataset',
        ...
        HR_2x_img, HR_2x_text, x2 = model(hr_img_x2, hr_text_x2)

        loss_img_x2 = criterion_zero(HR_2x_img, hr_img)
        loss_text_x2 = criterion_zero(HR_2x_text, hr_text)
        loss_x2_split = loss_img_x2 + loss_text_x2
        loss_x2_fusion = criterion(x2, HR_label)
        ...
        optimizer.step()
        if iteration % 200 == 0:
            print("===> Epoch[{}]({}/{}): loss_x2: {:.10f},loss: {:.10f}".format(epoch,
                                                                                 iteration,
                                                                                 len(
