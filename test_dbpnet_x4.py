...
parser.add_argument("--erm", action="store_true", default=True, help="use cuda?")
parser.add_argument("--model",
                    default="./models/x4/best_model.pth",
                    type=str)
parser.add_argument("--dataset", default="./dataset/test/total_crop/", type=str)
opt = parser.parse_args()

model = torch.load(opt.model, map_location=torch.device('cpu'))["model"]  # 使用cpu运行加载模型
...
for image_name in image_list:
    ...
    if opt.erm:
        HR_2x_text, HR_2x_img, HR_4x_text, HR_4x_img, x2, x4, edge = model(img_lr_y, text_lr_y)
    else:
        HR_2x_text, HR_2x_img, HR_4x_text, HR_4x_img, x2, x4 = model(img_lr_y, text_lr_y)
